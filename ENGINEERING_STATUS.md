...
**Date:** 2026-09-18
**Phase:** Engineering initialization + first vertical slice
...
$ npm run build         # dist/index.html 0.51 kB, index.css 5.91 kB, index.js 410.86 kB (gzip 90.88 kB)
...
2. **No authored content beyond the first slice.** `21_CONTENT_ATOMS` holds 20 templated rows
   with placeholder hooks and no question options, so it cannot drive a lesson. The two authored
   atoms and two activities in `data/authored/` are explicitly labelled prototype scaffolding.
3. **No AI generation layer.** The tutor runtime, router, ladder and gates are implemented and
...
4. **Translations exist for en + ko only.** 10 of 12 registered locales have 0% coverage.
   Filling them requires human translation and local-market review, not machine text.
5. **Persistence is in-memory.** Session state and mastery do not survive a reload; no database
   or auth has been chosen.
6. **No deployment target chosen.** `npm run build` produces a static `dist/`.
7. **npm audit reports 5 advisories** in the dev toolchain (Vite/esbuild dev-server class).
...
1. **Governance first (not code): approve a 5–10 node MVP evidence seed set.** Pick nodes whose
   claims are covered by the existing Tier-A evidence already in `14_EVIDENCE` (AAD-01…05,
   FDA, EU CosIng), verify them, and set `Status=Approved` + `Evidence_Status=Verified` **in
   the workbook**. Then `npm run extract:sources`. The publication gate will start returning
   `VERIFIED_FACT` with no code change. Until this happens, further content work is blocked.

2. **Decide OQ-L01 (locale set) and SR-001 (strand taxonomy).** Both are single-constant changes
   once decided, and both block content planning.

3. **Then, in code:** build Ingredient Garden on the newly approved nodes, which will exercise
   the `SCIENTIFIC` claim path end-to-end for the first time and prove the evidence chain from
   `14_EVIDENCE` through to rendered UI.

Exact next command once step 1 lands:
