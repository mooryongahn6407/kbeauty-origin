 ...
 *
 * Renders the reducer in src/app/learning-session.ts. The screen owns no learning logic:
 * it dispatches actions and displays whatever state and disclosures come back.
 *
...
export function MySkinScreen({ locale }: { locale: string }) {
  ...
  const atom = sliceAtoms()[0];
  const boundaryAtom = sliceAtoms()[1];
  ...
  return (
    <div className="stack">
      <section className="card">
        <p className="eyebrow">{grounding.questName} · {grounding.skillName}</p>
